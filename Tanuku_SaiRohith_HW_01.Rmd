...
```{r, solution_02e}
diamonds %>% 
  ggplot(mapping = aes(x = color, y = price)) + geom_boxplot(mapping = aes(fill = carat > median(carat))) + facet_grid(clarity~cut) +
...
Problem 2b) illustrated price summaries based solely on color. In this depiction, it seemed that the median price for "J" color was higher than that of "D" color. However, the figure generated in Problem 2f) reveals different trends that align more closely with expected behavior. For diamonds with carat weights below the median value, the optimal "D" color exhibits the highest median price among all colors. In fact, the 25th quantile of price for color == "D" surpasses that of all non-outlier diamonds for other color levels. For diamonds with carat weights exceeding the dataset's median value, the superior color grades ("D" and "E") boast median prices surpassing those of the lower-quality color grades. Therefore, it's crucial to consider carat weight when comparing diamond prices.
...
```
displ is significant because the confidence interval doesn't touch the line across 0
...
#### SOLUTION
All variables are significant because the confidence interval doesn't touch the line across 0, and there are 4 coefficents.

As shown by the graph below, all coefficients appear to be significant since none of the confidence intervals “contain 0”. We specified two variables in the formula interface to mod2. However, our plot shows 4 coefficients. The intercept, the coefficient on displ and then 2 coefficients that have drv in their name. We will learn why this is the case later in the semester. For now, all you needed to do was answer that there are 4 coefficients.
```{r, solution_04d}
coefplot::coefplot(mod2) +theme(legend.position = 'none')
```
You can remove the intercept from the following graph to have a better understanding of the predictor coefficients, although doing so is not necessary to be credited. The subsetting strategy covered in a previous problem can be applied here. By doing this, the bounds on the confidence interval linked to the displ coefficient become more visible.
```{r, solution_04dd}
coefplot::coefplot(mod2, coefficients = names(coef(mod2))[-1]) +
  theme(legend.position = 'none')
```
Else, we can set intercept = FALSE in the coefplot() call.
```{r, solution_04dddd}
...
$$
The LaTeX you need to type to render the equation above is shown below:
...
\left( X_{1}^{Z^{Y}} \right)
$$
The LaTeX you need to type to create the rendered equation above is shown below.
